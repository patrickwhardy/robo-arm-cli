...
// moves is an extensible object to make GamerInstance generic, moves stateless and adding moves simple
const moves = {
  start: {
    argCount: 1,
  ...
  replay: {
    argCount: 1,
    validate: (args, slots, executedCommands) => {
      // can't replay first command 'start'
    ...
    execute: (args, slots, executedCommands) => {
      ...
      for (let i = 0; i < args[0]; i++) {
        let nextCommand = executedCommands[executedCommands.length - (i + 1)]
...
function executeMove(move, args, slots) {
  ...
  // TODO: add validation w current state
